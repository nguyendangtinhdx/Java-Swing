...
public class DocGiaDAL extends DataAccessHelper{
    ...
    private final String ADD_DATA = "INSERT INTO DocGia(MaDG,TenDG,Khoa,SDT) VALUES (?,?,?,?)";
    private final String UPDATE_DATA = "UPDATE DocGia SET TenDG=?,Khoa=?,SDT=? WHERE MaDG=? ";
    private final String REMOVE_DATA = "DELETE FROM DocGia WHERE MaDG = ? ";
    ...
    public boolean AddData(DocGia dg) {
        boolean check = false;
        try {
            getConnect();
